...
class NewsDataLoader(TextClassificationRawDataLoader):
...
def add_args(parser):
    parser.add_argument('--data_dir', type=str, default='data/store/20news', help='data directory')
    parser.add_argument('--h5_file_path', type=str, default='data/store/20news/20news_data.h5',
...
if __name__ == "__main__":
    ...
    loader = NewsDataLoader(args.data_dir)
    loader.load_data()

    logger.info("Start generating h5 data file")
    start = time.time()
    loader.generate_h5_file(args.h5_file_path)
    end = time.time()
